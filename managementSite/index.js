...
async function callAPI(period, start, type) {
    try {
        ...
        let body;
        if (type == "done") {
            body = JSON.stringify({ "httpMethod": "GET_DONE_BINGO_NUMBER", "period": period, "start": now });
        } else if (type == "posted") {
            body = JSON.stringify({ "httpMethod": "GET_POSTED_BINGO_NUMBER", "period": period, "start": now });
        } else {
            return -1;
        }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    createGraph("Month", "done_month", "done", (new Date().getMonth() + 1).toString());
    createGraph("Year", "done_year", "done", new Date().getFullYear().toString());
    createGraph("Day", "posted_day", "posted", new Date().getDate().toString());
    createGraph("Month", "posted_month", "posted", (new Date().getMonth() + 1).toString());
    createGraph("Year", "posted_year", "posted", new Date().getFullYear().toString());
    get_number();
});
